...
void HashTable::Insert(Bid bid) {
    ...
    {
        Node* followingNode = new Node(bid, key);
        nodes.insert(nodes.begin() + key, (*followingNode));
    }
    else 
    {
...
void HashTable::PrintAll() {
    ...
    for (int i = 0; i < nodes.size(); i++) {
        if(nodes[i].key != UINT_MAX) {
            cout << "|Key: |" << nodes[i].key << "| BidId: |" << nodes[i].bid.bidId << "| Title: |" << nodes[i].bid.title << "| Fund: |" << nodes[i].bid.fund << endl;
        }
   }
}
